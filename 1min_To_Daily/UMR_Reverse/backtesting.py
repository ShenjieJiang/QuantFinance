...
def ic_and_pnl(df,factor,ic_type='IC',log=False,name=None):
    if log:
        calculate_and_plot_ic(df, factor=factor, ret='ret_daily', ic_type=ic_type,log=log,nameb=name+'//ic.png')
    else:
        calculate_and_plot_ic(df, factor=factor, ret='ret_daily', ic_type=ic_type)

    # 算benchmark
    dfa = calculate_benchmark_return(df, factor, 'ret_overnight', 'ret_day', 'ret_daily')

    dfa[factor] = dfa[factor].astype('float')

    # 分组
    dfa['quantile'] = dfa.groupby(['date'])[factor].transform(lambda x: pd.qcut(x,10,labels=False,duplicates='drop'))

    pnl_result = get_pnl(dfa)
    if log:
        plot_cumulative_returns(pnl_result.pivot(index='date', columns='quantile', values='cum_gross_excess'),log=log,name=name+'//cumulative.png')
    else:
    ...
    if log:
        ...
        result_table.dfi.export(name+'//pnl.png')
    else:
        display(calculate_annual_metrics(pnl_result)) 
